...
def run(params: Dict):
    ...
    interval_width = float(np.mean(upper_l - lower_l))
    interval_coverage = float(np.mean(catch_true_values))

    # Store the interval coverages and widths
    # Store the outputs in a dictionary
    bootstrap_data = {"Width of CIs - bootstraps": interval_width, "Coverage of CIs - bootstraps": interval_coverage}
    
    # Specify the full file path
    json_file_path = os.path.join(params['output_dir'], "CI_information_bootstraps.json")
